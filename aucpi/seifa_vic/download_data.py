...
def download_dataset(dataset, suffix, links, should_unzip=False):
	local_path = user_cache_dir('aucpi')/f"{dataset}.{suffix}"
	cached_download(links[dataset], local_path)
	if should_unzip == True:
		new_local_path = user_cache_dir('aucpi')/f"{dataset}_unzipped"
...
def load_csv_data(dataset='victorian_suburb_list'):
	links = get_data_links()
	# local_path = download_dataset(dataset, 'csv', links)
	return pd.read_csv(links[dataset])
...
def load_shapefile_data(dataset='seifa_2006_cd_shapefile'):
	...
	return gpd.read_fil(shpfile_path[0])
